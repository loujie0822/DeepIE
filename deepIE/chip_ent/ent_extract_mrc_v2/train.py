...
class Trainer(object):
    ...
    def forward(self, batch, chosen=u'train', eval=False, answer_dict=None):
        ...
        if not eval:
            input_ids, token_type_ids, segment_ids, pos_starts, pos_ends, pos_spans, pos_spans_mask, is_impossibles, query_ids = batch
            loss = self.model(input_ids=input_ids, token_type_ids=segment_ids, span_label_mask=pos_spans_mask,
                              object_labels=(pos_starts, pos_ends, pos_spans, is_impossibles))
            if self.n_gpu > 1:
            ...
            p_ids, input_ids, token_type_ids, segment_ids, query_ids = batch
            eval_file = self.eval_file_choice[chosen]
            po_pred = self.model(input_ids=input_ids,
                                 token_type_ids=segment_ids,
                                 is_eval=eval)
            #start_labels, end_labels, span_scores = po_pred
            ans_dict = self.convert_spo_contour(p_ids, po_pred, query_ids, eval_file, answer_dict)
            return ans_dict
    ...
    def convert_spo_contour(self, qids, po_preds, query_ids, eval_file, answer_dict):
        qids = qids.data.cpu().numpy()
        starts = po_preds[0].data.cpu().numpy()
        ends = po_preds[1].data.cpu().numpy()
        spans = po_preds[2].data.cpu().numpy()
        query_ids = query_ids.data.cpu().numpy()

        for qid, start, end, span, query_id in zip(qids, starts, ends, spans, query_ids):
            ...
            start = np.where(start == 1)[0]
            end = np.where(end == 1)[0]
            ...
            for _start in start:
                if _start > len(tokens) - 2 or _start == 0:
                    continue
                for _end in end:
                    if _start <= _end <= len(tokens) - 2:
                        po_index_lst.append((_start, _end, query_id))
                        break
            po_lst = []
