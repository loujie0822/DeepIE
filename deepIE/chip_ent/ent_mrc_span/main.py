...
from deepIE.chip_ent.ent_mrc_span.data_loader_char import Reader, Feature
from deepIE.chip_ent.ent_mrc_span.train import Trainer
...
def get_args():
    ...
    args.cache_data = args.input + '/{}_mrc_span_cache_data_{}/'.format(str(args.bert_model).split('/')[1],
                                                                            str(args.max_len))
    return args
...
def bulid_dataset(args, spo_config, reader, tokenizer, debug=False):
    ...
    dev_examples = dev_examples[:9] if debug else dev_examples
