...
from deepIE.config.config import CMeEnt_Query
...
class Reader(object):
    ...
    def _read(self, filename, data_type):
                ...
                for i, text in enumerate(text_lst):
                    ...
                    for (start, end, ent_name_, ent_type) in ent_lst:
                        if (i * self.max_seq_length) <= start < ((i + 1) * self.max_seq_length) and (
                            ...
                            type_dict[ent_type].append((start - i * self.max_seq_length, end - i * self.max_seq_length,
                    ...
                    for ent_type, query in CMeEnt_Query.items():
                        po_list = type_dict.get(ent_type, [])
                        query = [c.lower() for c in query]
                        query = query + ["[SEP]"]
                        segment = segment_ + len(query) * [1]
                        examples.append(
                            Example(
                                p_id=p_id,
                                text_id=text_id,
        ...
        logging.info('total size after split in {} is {}'.format(data_type, after_text_num))
    ...
    def _create_collate_fn(self):
        def collate(examples):
            ...
            for example in examples:
                token_ids = self.tokenizer.encode(example.bert_tokens + example.query)[1:-1]
                token_type_ids = np.zeros(len(token_ids), dtype=np.long)
                segment_ids = example.segment
...
def span_padding(seqs, span_lable, is_float=False, class_num=None):
    ...
    span_tensor = torch.FloatTensor(len(seqs), batch_length, batch_length).fill_(float(0)) if is_float \
        else torch.LongTensor(len(seqs), batch_length, batch_length).fill_(0)

    for i, po_list in enumerate(span_lable):
        for po in po_list:
            start_pos = po[0] + 1
            end_pos = po[1] + 1
            span_tensor[i, start_pos, end_pos] = 1
