...
from deepIE.config.config import Ent_BIO
            ...
            for line in tqdm(fr.readlines()):
                ...
                for spo in spo_list:
                    ...
                    if subject_start != -1 and object_start != -1:
                        ...
                        bio[subject_start] = 'B'
                        bio[subject_start + 1: subject_start + len(subject_sub_tokens)] = 'I' * (
                                len(subject_sub_tokens) - 1)
                        bio[object_start] = 'B'
                        bio[object_start + 1: object_start + len(object_sub_tokens)] = 'I' * (
                                len(object_sub_tokens) - 1)

                        for index, label_ in enumerate(bio):
                            ent_labels[index] = Ent_BIO[label_]
    ...
    def _create_collate_fn(self):
        def collate(examples):
            ...
            for example in examples:
                ...
                token_ids = self.tokenizer.encode(bert_tokens)
