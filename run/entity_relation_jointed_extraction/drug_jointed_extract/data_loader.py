...
from utils.data_util import Tokenizer, sequence_padding
...
class Reader(object):
    def __init__(self, do_lowercase=False, seg_char=False):
        ...
        self.relation_config = DRUG_RELATION
        ...
        with codecs.open(filename, 'r') as f:
            ...
            for line in tqdm(f):
                ...
                for spo in data_json['spo_list']:
                    subject_name = spo['subject'][0]
                    object_name = spo['object'][0]

                    spo_list.append((subject_name, spo['subject'][1], spo['predicate'], object_name, spo['object'][1],
                                     spo['object'][2])
                                    )
                for ent in data_json['ent_list']:
                    ent_list.append((ent[1], ent[2]))

                if data_type == 'train':
                    spoes = {}
                    for spo_dict in data_json['spo_list']:
                        sub_ = spo_dict['subject']
                        p = spo_dict['predicate']
                        obj_ = spo_dict['object']
                        p = DRUG_RELATION[p]
                    ...
                    for ent in data_json['ent_list']:
                        examples.append(
                            Example(
                                ...
                                context=text,
                                text_word=text_word,
                                sub_entity_list=(ent[1], ent[2] - 1),
                                gold_answer=spo_list,
                                spo_list=data_json['spo_list'],
                                ent_list=ent_list,
                                spoes=spoes,
                            )
                        )
