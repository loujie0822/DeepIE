...
class ERENet(nn.Module):
    ...
    def __init__(self, args, word_emb, spo_conf):
        ...
        self.second_sentence_encoder = SentenceEncoder(args, args.hidden_size * 2)
        # self.second_sentence_encoder = SentenceEncoder(args, args.hidden_size)
        self.token_entity_emb = nn.Embedding(num_embeddings=2, embedding_dim=args.hidden_size * 2,
                                             padding_idx=0)
    ...
    def forward(self, q_ids=None, char_ids=None, word_ids=None, token_type_ids=None, subject_ids=None,
        ...
        sent_encoder = self.first_sentence_encoder(char_emb, seq_mask)

        if not is_eval:
            subject_encoder = self.token_entity_emb(token_type_ids)
            context_encoder = sent_encoder + subject_encoder
            context_encoder = self.second_sentence_encoder(context_encoder, seq_mask)
            ...
            token_type_ids = torch.cat(token_type_ids).to(sent_encoder.device)
            ...
            for i in range(num):
                ...
                subject_encoder = self.token_entity_emb(token_type_ids)
                context_encoder = sent_encoders + subject_encoder
                context_encoder = self.second_sentence_encoder(context_encoder, pass_ids.eq(0))
