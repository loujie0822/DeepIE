...
from config.spo_config_v1 import BAIDU_BIES
from run.entity_relation_jointed_extraction.etl_bies.data_loader import Reader, Vocabulary, Feature
from run.entity_relation_jointed_extraction.etl_bies.train import Trainer
...
def get_args():
    ...
    if args.use_word2vec:
        args.cache_data = args.input + '/w2v_bies_cache_data/'
    elif args.use_bert:
        args.cache_data = args.input + '/bert_bies_cache_data/'
    else:
...
def bulid_dataset(args, reader, vocab, debug=False):
    ...
    train_examples = train_examples[:5] if debug else train_examples
    dev_examples = dev_examples[:5] if debug else dev_examples
...
def main():
    ...
    trainer = Trainer(args, data_loaders, eval_examples, word_emb, spo_conf=BAIDU_BIES)
