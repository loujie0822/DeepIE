...
from run.entity_extraction.crf.data_loader import Reader, Vocabulary, config, Feature
from run.entity_extraction.crf.train import Trainer
...
def get_args():
    ...
    if args.use_word2vec:
        args.cache_data = args.input + '/w2v_cache_data/'
    elif args.use_bert:
        args.cache_data = args.input + '/bert_cache_data/'
    else:
...
def bulid_dataset(args, reader, vocab, debug=False):
    ...
    convert_examples_features = Feature(args, token2idx_dict=char2idx,entity_type=args.entity_type)

    train_examples = train_examples[:4] if debug else train_examples
    dev_examples = dev_examples[:4] if debug else dev_examples
...
def main():
    ...
    trainer = Trainer(args, data_loaders, eval_examples, char_emb, ent_conf=config[args.entity_type])
