...
def data_initialization(data, train_file, dev_file, test_file):
    if data.transfer:
        data.build_alphabet(train_file, only_label=True)
        data.build_alphabet(dev_file, only_label=True)
        data.build_alphabet(test_file, use_label=False)
    else:
...
def batchify_with_label(input_batch_list, gpu, device):
    ...
    if gpu:
        ...
        biword_seq_tensor = biword_seq_tensor.cuda(device)
        word_seq_lengths = word_seq_lengths.cuda(device)
...
def train(data, save_model_dir, seg=True, debug=False, transfer=False):
    ...
    if data.bert_finetune:
        ...
        model = SeqModel(data)
        if transfer:
            print('transfer go on !!!')
            model_dict = torch.load(save_model_dir, map_location='cpu')
